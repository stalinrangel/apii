...
class NotificacionController extends Controller
{
        ...
        if(count($pedido)==0){
            ...
            if (count($repartidores) > 1) {
                ...
                if ($repartidores[0]->usuario->token_notificacion) {
                    $this->enviarNotificacion($repartidores[0]->usuario->token_notificacion, 'Tienes%20un%20nuevo%20solicitud%20S00'.$pedido->id, $pedido->id, 1);
                }
        ...
        if ($bandera)
        {
            ...
            if ($pedido->save() && $repartidor->save()) {
                ...
                if ($repartidor->usuario->token_notificacion) {
                    $this->enviarNotificacion($repartidor->usuario->token_notificacion, 'Se%20te%20ha%20asignado%20un%20solicitud%20S00'.$pedido_id, $pedido->id, 8);
                }
                ...
                if ($notificarCliente) {
                    ...
                    if ($pedido->usuario->tipo_usuario == 2) {
                        ...
                        if ($pedido->usuario->token_notificacion) {
                            $this->enviarNotificacionCliente($pedido->usuario->token_notificacion, 'Tu%20solicitud%20S00'.$pedido_id.'%20ha%20sido%20aceptada.', $pedido->id, 7);
                        }
                    }
                    //En caso de que el pedido lo haga un establecimiento
    ...
    public function finalizarPedido(Request $request, $repartidor_id)
    {
        ...
        $this->enviarNotificacion($pedido->usuario->token_notificacion, 'El%20cliente%20ha%20finalizado%20la%20solicitud%20S00'.$request->input('pedido_id').'.', $request->input('pedido_id'), 16,$obj);

        $this->enviarNotificacionCliente($pedido->usuario->token_notificacion, 'Se%20ha%20finalizado%20la%20solicitud%20S00'.$request->input('pedido_id').'.', $request->input('pedido_id'), 3,$obj);

         $obj = array('pedido_id'=>$pedido->id, 'usuario_id'=>$pedido->usuario->id);

        if ($request->input('finalizo')==3) {
            $this->enviarNotificacion($pedido->usuario->token_notificacion, 'El%20cliente%20ha%20finalizado%20la%20solicitud%20S00'.$request->input('pedido_id').'.', $pedido->id, 16,$obj);
